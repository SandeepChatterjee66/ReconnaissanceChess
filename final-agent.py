...
STOCKFISH_ENV_VAR = '/home/chatterjee/Codes/Reinforecement Learning/r-chess/stockfish/stockfish-ubuntu-x86-64-avx2'

def partition (list_in, n):
	random.shuffle(list_in)
	return [list_in[i::n] for i in range(n)]

class Final_Agent(Player):
	def __init__(self):
		self.color = None
		self.moveNum = 0
		self.approxTime = 900
		...
		if STOCKFISH_ENV_VAR not in os.environ:
			raise KeyError(
				'Final_Agent requires an environment variable called "{}" pointing to the Stockfish executable'.format(
					STOCKFISH_ENV_VAR))
	...
	def handle_game_end(self, winner_color: Optional[Color], win_reason: Optional[WinReason],
		...
		if self.color == chess.WHITE:
			white_bot_name = "Final_Agent"
			black_bot_name = self.enemy
		else:
			black_bot_name = "Final_Agent"
...
import os
from reconchess import play_local_game, load_player
# Ensure that the stockfish executable path is set correctly for Final_Agent
STOCKFISH_PATH = '/home/chatterjee/Codes/Reinforecement Learning/r-chess/stockfish/stockfish-ubuntu-x86-64-avx2'
os.environ['/home/chatterjee/Codes/Reinforecement Learning/r-chess/stockfish/stockfish-ubuntu-x86-64-avx2'] = STOCKFISH_PATH

# Load two players: Final_Agent and an opponent (either another instance of Final_Agent or a simple AI)
def load_stocky_inference():
    return Final_Agent()

# This function will play a local game and print out results
def play_game():
    # Load the players
    player1 = load_stocky_inference()
    player2 = load_stocky_inference()  # Replace with another Player instance if desired

    # Start the game
    print("Starting Reconnaissance Blind Chess game...")
    winner, win_reason, history = play_local_game(player1, player2)
    ...
    # Optionally, print or save the move history
    print("Game history:")
    for move in history:
        print(move)
